...
specan3 <- function(X, bp = c(0,22), wl = 2048, threshold = 5, parallel = 1){
  ...
  x <- as.data.frame(lapp(1:length(start), function(i) { 
    r <- tuneR::readWave(file.path(sound.files[i]), from = start[i], to = end[i], units = "seconds") 
...
readWavFile <- function(fileName) {
  ...
  folderName <- paste(tokens[-length(tokens)], collapse="/")
...
get_gender <- function(fileName){
  ...
  #load model
  if (!exists('genderSvm')) {
    # saveRDS(genderSvm, "genderSvm.rds")
    genderSvm <- readRDS( "genderSvm.rds")
  }
  
  #genderSvm
  predictSvm <- predict(genderSvm, data)
  return(as.character(predictSvm))
}

# wavFile = "xxx.wav"
# print(get_gender(wavFile))
