...
def extract_entities(query_str):
    ...
    print(intent)
    print(confidence)
    if '装' in query_str and '电视' in query_str:
        return "install","install"
    if intent == "install_place" and confidence > 0.2:
    ...
    if intent == "date":
       values = [e["value"] for e in parsed_j["entities"]]
       return intent,values[0]
    if 'LCD' in tmp and '看看' not in query_str :
       if len(getAllNumber(query_str)) == 0:
          return "getsize","getsize"
    if intent == "install" and confidence > 0.2:
       return intent,intent
    if '到家了吗' in tmp:
       if '没' in query_str or '快' in query_str:
          return "notArrive","notArrive"
    if '电视信号' in tmp:
       if '没' in query_str:
          return "notSignal","notSignal"
    if intent == "phone_number" or intent == "install_type":
        ...
        if len(number) >= 10:
         ...
         #print(intent)
    if intent == "name" and confidence > 0.2:
        return intent,intent 
    if(intent == "greet"):
